...
the DOCX renderer this can't preserve the original font — insert_text only
supports PyMuPDF's built-in Base-14 fonts, so any embedded/subsetted font in
the source PDF (the common case) is substituted, not reproduced. That's an
accepted, surfaced limitation (see the structure-preserving resume plan) —
every substituted or overflow-truncated block is reported back via
`low_confidence_block_ids` rather than silently degrading.
...
def _write_block(page: fitz.Page, block: TextBlock) -> bool:
    ...
    width_budget = anchor.x1 - anchor.x0

    text, fontsize, truncated = _fit_text(block.text, fontname, original_size, width_budget)
    page.insert_text((anchor.x0, anchor.y1), text, fontname=fontname, fontsize=fontsize)

    if truncated:
        logger.warning("Truncated block %s to fit its original width", block.block_id)
...
def _fit_text(text: str, fontname: str, fontsize: float, width_budget: float) -> tuple[str, float, bool]:
    """Shrink font size to fit width_budget before ever truncating text."""
    size = fontsize
    while size >= _MIN_FONT_SIZE:
        if fitz.get_text_length(text, fontname=fontname, fontsize=size) <= width_budget:
            return text, size, False
        size -= _SHRINK_STEP

    truncated = _truncate_to_width(text, fontname, _MIN_FONT_SIZE, width_budget)
    return truncated, _MIN_FONT_SIZE, True


def _truncate_to_width(text: str, fontname: str, fontsize: float, width_budget: float) -> str:
    if fitz.get_text_length(text, fontname=fontname, fontsize=fontsize) <= width_budget:
        return text

    truncated = text
    while truncated and fitz.get_text_length(truncated + _ELLIPSIS, fontname=fontname, fontsize=fontsize) > width_budget:
        truncated = truncated[:-1]
    return (truncated + _ELLIPSIS) if truncated else _ELLIPSIS
